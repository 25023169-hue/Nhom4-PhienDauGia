...
public class UserProfileEndpointController {
    ...
    @Autowired
    private UserProfileLogicService userProfileLogicService;

    @PutMapping("/{id}/name")
    public ResponseEntity<?> updateProfileName(@PathVariable("id") Long id, @RequestBody Map<String, String> payload) {
        try {
