  ...
  public RevenueStatsDTO getSellerRevenueStats(Long sellerId) {
    ...
    for (Transaction tx : sales) {
      ...
      if (revenueByMonth.containsKey(month)) {
        revenueByMonth.put(
            month, revenueByMonth.get(month) + (tx.getMoneyIn() == null ? 0.0 : tx.getMoneyIn()));
      }
    }

    DateTimeFormatter monthFormatter = DateTimeFormatter.ofPattern("MM/yyyy");
    List<ChartPointDTO> monthlyRevenue = new ArrayList<>();
    revenueByMonth.forEach(
        (month, revenue) ->
            monthlyRevenue.add(new ChartPointDTO(month.format(monthFormatter), revenue)));

    List<TransactionDTO> recentSales = sales.stream().limit(20).map(this::toDTO).toList();

    return new RevenueStatsDTO(
        totalRevenue, monthRevenue, soldOrders, averageOrderValue, monthlyRevenue, recentSales);
  }
