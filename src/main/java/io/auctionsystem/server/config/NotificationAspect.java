...
public class NotificationAspect {
    ...
    @AfterReturning(pointcut = "execution(* io.auctionsystem.server.service.BidService.placeBid(io.auctionsystem.common.request.BidRequest)) && args(request)", returning = "result")
    public void afterBidPlaced(BidRequest request, Object result) {
        ...
        if (result instanceof BidResponse) {
            ...
            if (response.isAccepted()) {
                String message = "Bạn đã đặt giá thành công mức " + response.getNewCurrentPrice() + " VNĐ cho phiên đấu giá #" + response.getAuctionId();
