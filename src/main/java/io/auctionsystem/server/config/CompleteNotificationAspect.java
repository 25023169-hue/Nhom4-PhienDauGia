...
public class CompleteNotificationAspect implements InitializingBean {
  ...
  public CompleteNotificationAspect(
  ...
  public Object handleBidNotifications(ProceedingJoinPoint joinPoint, BidRequest request)
      throws Throwable {
    Object result = joinPoint.proceed();
    if (!(result instanceof BidResponse response) || !response.isAccepted()) {
      return result;
    }
  ...
  @AfterReturning(
      pointcut =
          "execution(* io.auctionsystem.server.service.AuctionSettlementService.closeExpiredAuction(..))"
              + " && args(auctionId, closedAt)",
      returning = "closed")
  public void handleExpiredAuction(Long auctionId, LocalDateTime closedAt, boolean closed) {
    if (closed) {
  ...
  @AfterReturning(
      pointcut =
          "execution(* io.auctionsystem.server.service.AuctionSettlementService.closeBuyNowAuction(..))"
              + " && args(auctionId)",
      returning = "closed")
  public void handleBuyNowAuction(Long auctionId, boolean closed) {
    if (closed) {
  ...
  @AfterReturning(
      pointcut =
          "execution(* io.auctionsystem.server.service.AuctionSettlementService.cancelAuction(..))"
              + " && args(auctionId)",
      returning = "cancelled")
  public void handleCancelledAuction(Long auctionId, boolean cancelled) {
    if (cancelled) {
