...
public class BankSettingController {
    ...
    public void initialize() {
        ...
        cbBankName.addEventFilter(KeyEvent.ANY, event -> {
            ...
            if (isSpace) {
                ...
                } else if (event.getEventType() == KeyEvent.KEY_RELEASED || event.getEventType() == KeyEvent.KEY_TYPED) {
                    event.consume();
                }
            }
        });
        ...
        cbBankName.getEditor().textProperty().addListener((observable, oldValue, newValue) -> {
            ...
            List<String> filtered = BANK_LIST.stream()
            ...
            if (!currentBanks.equals(filtered)) {
                ...
                if (cbBankName.getEditor().isFocused()) {
                    ...
                    boolean exactMatch = BANK_LIST.stream().anyMatch(bank -> bank.equalsIgnoreCase(keyword));
        ...
        cbBankName.getEditor().setOnMouseClicked(e -> {
            if (cbBankName.getEditor().isFocused() && !currentBanks.isEmpty() && !cbBankName.isShowing()) {
                cbBankName.setVisibleRowCount(Math.min(10, Math.max(1, currentBanks.size())));
                cbBankName.show();
            }
        });
        ...
        cbBankName.valueProperty().addListener((observable, oldValue, newValue) -> {
        ...
        new Thread(() -> {
            try {
                ...
                HttpRequest request = HttpRequest.newBuilder()
                        .uri(URI.create("http://localhost:8080/api/user/" + user.getUserId() + "/bank"))
