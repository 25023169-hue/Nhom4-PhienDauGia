...
public class NotificationController {
            ...
            try {
                ...
                Platform.runLater(() -> {
                    if (response.statusCode() == 200) {
                        try {
                            List<NotificationDTO> notis = objectMapper.readValue(
                                    response.body(),
    ...
    private void markAsRead(NotificationDTO notification) {
        new Thread(() -> {
            try {
                HttpRequest request = HttpRequest.newBuilder()
                        .uri(URI.create("http://localhost:8080/api/notifications/" + notification.getNotiId() + "/read"))
    ...
    private static class NotificationCell extends ListCell<NotificationDTO> {
        ...
        protected void updateItem(NotificationDTO notification, boolean empty) {
            ...
            title.setStyle("-fx-font-weight: " + (notification.isRead() ? "normal" : "bold") + "; -fx-text-fill: #2c3e50;");

            String time = notification.getCreatedAt() == null ? "" : notification.getCreatedAt().format(DISPLAY_FORMATTER);
            String type = notification.getType() == null || notification.getType().isBlank()
                    ? "Thông báo"
