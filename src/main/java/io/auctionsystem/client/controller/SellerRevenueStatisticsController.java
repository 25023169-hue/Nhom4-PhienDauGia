...
public class SellerRevenueStatisticsController {
        ...
        new Thread(() -> {
            try {
                HttpRequest request = HttpRequest.newBuilder()
                        .uri(URI.create("http://localhost:8080/api/user/" + sellerId + "/revenue-stats"))
                        .GET()
                        .build();
    ...
    private void renderRevenueStats(String responseBody) {
        try {
            ...
            renderChart(objectMapper.convertValue(root.path("monthlyRevenue"), new TypeReference<List<Map<String, Object>>>() {}));
    ...
    private void renderRecentSales(JsonNode salesNode) {
        ...
        for (JsonNode sale : salesNode) {
            ...
            recentSales.add(new TransactionModel(
                    time,
