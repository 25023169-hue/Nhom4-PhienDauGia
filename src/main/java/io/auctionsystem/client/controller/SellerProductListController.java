...
public class SellerProductListController {
  ...
  public void initialize() {
    ...
    colStartingPrice.setCellFactory(
        column ->
            new TableCell<>() {
              @Override
  ...
  private String buildListUrl(Long sellerId) {
    StringBuilder url =
        new StringBuilder(Constants.BASE_URL).append("/seller/products?sellerId=").append(sellerId);
  ...
  private void handleLoadResponse(HttpResponse<String> response) {
    if (response.statusCode() == 200) {
      try {
        List<SellerProductDTO> loadedProducts =
            objectMapper.readValue(response.body(), new TypeReference<List<SellerProductDTO>>() {});
  ...
  private void updateSummary(List<SellerProductDTO> loadedProducts) {
    long openCount =
        loadedProducts.stream().filter(product -> product.getStatus() == AuctionState.OPEN).count();
    long runningCount =
  ...
  private void applyLoadedProducts(List<SellerProductDTO> loadedProducts) {
    ...
    synchronized (pendingCreatedProducts) {
      pendingCreatedProducts.removeIf(
          pending -> loadedProducts.stream().anyMatch(loaded -> sameProduct(loaded, pending)));
  ...
  private boolean matchesSearch(SellerProductDTO product) {
    ...
    String itemName =
        product.getItemName() == null ? "" : product.getItemName().toLowerCase(Locale.ROOT);
